...
function verifyJWT(req, res, next) {
    ...
    const index = listaPreta.findIndex(item => item === token)

    if(index !== -1) { //encontrou o token na listaPreta
        return res.status(401).send(' index !== 1 user not found')
    }

    jwt.verify(token, SECRET, (err, decoded) => {
        if(err) { // token ausente ou token invalido
            return res.status(401).send('user not found jwt.verify');
        }
        // caso contrario
...
router.get('/acesso', verifyJWT ,(req, res) => {
   ...
   // res.json({id: 1, nome: 'Gustavo'})
    res.send('Usuario autorizado em /acesso')
})
...
router.post('/',(req, res) => {
    ...
    if(req.body.usuario === 'gustavo' && req.body.senha === '123456') {
        ...
        const token =  jwt.sign({userId: 1}, SECRET, {expiresIn: '1hr'})
...
xhr.send(JSON.stringify({ password: hash }));
