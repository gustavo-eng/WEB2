...
// Rota.get('/', async ({response}) => {
//   console.log('Acessando rota /jogo por Routes.js ')  
//   console.log(response.send('Ola mundo'))
// })
...
Route.get('/', async ({response}) => {
  // run(1)
  consoles()
  console.log(Application.viewsPath('providers/imgAdonis.png'))
  // const filePath = Application.tmpPath('./providers/imgAdonis.png')
  response.send(' ola ')
  // response.download('./providers/imgAdonis.png')
  // response.send('ola')
...
import { CannotWriteFileException, Response } from '@adonisjs/core/build/standalone'
...
Route.get('/parametros/:nome/:sobrenome', async ({ params, request }) => {
  console.log(params.nome)
  console.log('___________')
  // console.log(request.params().nome)
  console.log('00')
  console.log(request.param( 'sobrenome')) // pega um unico parametro especifico 
  console.log('params ')
  console.log(request.params()) // retorna 
  console.log(request.params().sobrenome)
  console.log('URL URL  -----> ')
  const url = Route.makeUrl('/parametros/:nome/:sobrenome', { nome: 'Gustavo', sobrenome: 'Alexandre' })
  console.log(url)
...
// })
Route.group(() => {
  Route.get('/adonis', async ({subdomains, response}) => {
    console.log('OLAA')
    console.log(subdomains.group)
    return response.send({ group: '  TRABALHO ADONIS   ' })
    
  })
}).domain(':group.adonisjs.com') 


Route.get('/docs/*', ({params, request}) => {
  console.log('/docs ==> ')
  console.log(params['*'])
  if (request.matchesRoute('/docs')) {
...
// /:category/sexo/peso/idade 
// /:category/time/divisao/idade_media
//Ao inves de ficar colocando nome de cada parametro, passa assim e faz uma validacao com retorno 
...
Route
  ...
  .middleware(async (contexto, next) => {
    console.log(`Inside middleware ${contexto.request.url()}`)
    await next()
  })
